...
fun List<Order>.getProductsSet(): Set<Product> = this.filter { it.isDelivered }.flatMap { it.products }.toSet()
...
fun Order.sumProductPrice(): Double = this.products.sumOf(Product::price)

// TODO: Return the product with the maximum price in the order
fun Order.getMaxPriceProduct(): Product =
  this.products.maxByOrNull { it.price }?:throw NoSuchElementException("No max price available for this order")

// TODO: Return the product with the min price in the order
fun Order.getMinPriceProduct(): Product =
  this.products.minByOrNull { it.price }?:throw NoSuchElementException("No min price available for this order")
